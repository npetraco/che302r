...
compute_hue <- function(xc, yc, xw=0.31, yw=0.33){
  ...
  angs <- rep(0, nrow(ciexyz1931))
  prjs <- rep(0, nrow(ciexyz1931))
  for(i in 1:nrow(ciexyz1931)){
    rs      <- c(x31[i],y31[i]) - wp
  ...
  lambda.hue <- ciexyz1931[mind.idx,1]
  xh         <- x31[mind.idx]
  yh         <- y31[mind.idx]
...
#' @param left.purple.vertex  A left purple vertex. Default is the one in ciexyz1931
#' @param right.purple.vertex A right purple vertex. Default is the one in ciexyz1931
#'
...
plot_spectral_locus <- function(Y.level, white.point=c(0.31,0.33), type="sRGB"){
  ...
  if(type=="Adobe") {
    col.vec <- sapply(1:nrow(ciexyz1931), function(xx){XYZ2Adobe(xyY2XYZ(c(x31[xx], y31[xx], Y.level)))$hex})
  } else if(type=="sRGB") {
    col.vec <- sapply(1:nrow(ciexyz1931), function(xx){XYZ2sRGB(xyY2XYZ(c(x31[xx], y31[xx], Y.level)))$hex})
  } else if(type=="P3") {
    col.vec <- sapply(1:nrow(ciexyz1931), function(xx){XYZ2P3(xyY2XYZ(c(x31[xx], y31[xx], Y.level)))$hex})
  } else if(type=="rough.hue") {
    col.vec <- sapply(1:nrow(ciexyz1931), function(xx){color_partition(wavelength = ciexyz1931[xx,1])})
  } else if(type=="wl2c") {
    col.vec <- sapply(1:nrow(ciexyz1931), function(xx){wavelengthToColor(wavelength = ciexyz1931[xx,1])})
  } else {
  ...
  #col.info.mat <- data.frame(ciexyz1931[,1:3], as.character(col.vec))
